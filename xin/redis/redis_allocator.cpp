...
void default_oom_handler(allocator::Size size)
{
...
auto assign(allocator::Size size) -> allocator::Size
{
...
std::atomic<allocator::Size> allocator::used_memory_{ 0 };

allocator::ErrorHandler allocator::error_handler_ = default_oom_handler;

auto allocator::malloc(Size size) -> void*
{
...
auto allocator::calloc(Size size) -> void*
{
...
auto allocator::size(void* ptr) -> Size
{
    auto* real_ptr = static_cast<char*>(ptr) - PREFIX_SIZE;
    auto size = *reinterpret_cast<Size*>(real_ptr);
    return assign(size) + PREFIX_SIZE;
}

auto allocator::realloc(void* ptr, Size size) -> void*
{
    if (!ptr)
        return malloc(size);

    auto* real_ptr = static_cast<char*>(ptr) - PREFIX_SIZE;
    auto old_size = *reinterpret_cast<Size*>(real_ptr);
    auto* new_ptr = std::realloc(real_ptr, size + PREFIX_SIZE);
    if (!new_ptr)
        error_handler_(size);

    *static_cast<Size*>(new_ptr) = size;
...
auto allocator::used_memory() -> Size { return used_memory_.load(std::memory_order_relaxed); }

void allocator::on_error(ErrorHandler handler) { error_handler_ = std::move(handler); }

void allocator::append_used_memory(Size size)
{
    used_memory_.fetch_add(assign(size), std::memory_order_relaxed);
}

void allocator::remove_used_memory(Size size)
{
