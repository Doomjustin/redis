...
TEST_SUITE("redis-command-hash")
{
    ...
    TEST_CASE("hset and hget basic behavior")
    {
        ...
        CHECK(response_to_string(hash_table_commands::set(
                  application_context::db(db_index), Arguments{ "HSET", "__hash_k1__", "name", "tom", "age", "10" })) ==
              ":2\r\n");
        CHECK(response_to_string(hash_table_commands::get(
                  application_context::db(db_index), Arguments{ "HGET", "__hash_k1__", "name" })) == "$3\r\ntom\r\n");
        CHECK(response_to_string(hash_table_commands::get(
                  application_context::db(db_index), Arguments{ "HGET", "__hash_k1__", "missing" })) == "$-1\r\n");
    }
    ...
    TEST_CASE("hset update returns number of newly added fields")
    {
        application_context::db(db_index).flush();
        CHECK(response_to_string(hash_table_commands::set(
                  application_context::db(db_index), Arguments{ "HSET", "__hash_k2__", "name", "tom" })) == ":1\r\n");

        CHECK(response_to_string(hash_table_commands::set(
                  application_context::db(db_index), Arguments{ "HSET", "__hash_k2__", "name", "jerry", "city", "sh" })) ==
              ":1\r\n");
    }
    ...
    TEST_CASE("hgetall returns alternating field-value array")
    {
        application_context::db(db_index).flush();
        CHECK(response_to_string(hash_table_commands::set(
                  application_context::db(db_index), Arguments{ "HSET", "__hash_k3__", "f1", "v1", "f2", "v2" })) ==
              ":2\r\n");

        auto res = response_to_string(
            hash_table_commands::get_all(application_context::db(db_index), Arguments{ "HGETALL", "__hash_k3__" }));
    ...
    {
        CHECK(
            response_to_string(hash_table_commands::set(application_context::db(db_index), Arguments{ "HSET", "k", "f" })) ==
            "-ERR wrong number of arguments for 'hset' command\r\n");
        CHECK(response_to_string(hash_table_commands::get(application_context::db(db_index), Arguments{ "HGET", "k" })) ==
              "-ERR wrong number of arguments for 'hget' command\r\n");
        CHECK(response_to_string(
                  hash_table_commands::get_all(application_context::db(db_index), Arguments{ "HGETALL", "k", "x" })) ==
              "-ERR wrong number of arguments for 'hgetall' command\r\n");

        application_context::db(db_index).flush();
        CHECK(response_to_string(string_commands::set(
                  application_context::db(db_index), Arguments{ "SET", "__hash_wrongtype_k1__", "v1" })) == "+OK\r\n");
        CHECK(response_to_string(hash_table_commands::get(
                  application_context::db(db_index), Arguments{ "HGET", "__hash_wrongtype_k1__", "f" })) ==
