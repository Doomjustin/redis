...
auto create_new_hash(Database& db, const Arguments& args) -> ResponsePtr
{
...
auto add_to_existing_hash(const Arguments& args, HashType& container) -> ResponsePtr
{
    ...
    log::debug("HSET command executed with key: {}, updated {} fields, added {} new fields",
               args[1], container.size() - new_fields, new_fields);
    return std::make_unique<IntegralResponse>(new_fields);
}
...
auto get_from_hash(const Arguments& args, HashType& container) -> ResponsePtr
{
    ...
    if (iter == container.end()) {
        log::debug("HGET command executed with key: {}, field: {}, but field does not exist",
                   args[1], field);
        return std::make_unique<NullBulkStringResponse>();
    }

    log::debug("HGET command executed with key: {}, field: {}, value found", args[1], field);
...
auto get_all_from_hash(const Arguments& args, HashType& container) -> ResponsePtr
{
    ...
    for (const auto& [field, value] : container) {
        log::debug("HGETALL command executed with key: {}, field: {}, value: {}", args[1], field,
                   *value);
        response->add_record(std::make_shared<std::string>(field));
        response->add_record(value);
    }

    log::debug("HGETALL command executed with key: {}, total {} fields returned", args[1],
               response->size() / 2);
    return response;
}
...
auto hash_table_commands::set(Database& db, const Arguments& args) -> ResponsePtr
{
    ...
    if (!res)
        return create_new_hash(db, args);
    ...
    log::info("HSET command executed with key: {}, but WRONGTYPE Operation against a key holding "
...
auto hash_table_commands::get(Database& db, const Arguments& args) -> ResponsePtr
{
    if (args.size() != 3) {
    ...
    log::info("HGET command executed with key: {}, field: {}, but WRONGTYPE Operation against a "
...
auto hash_table_commands::get_all(Database& db, const Arguments& args) -> ResponsePtr
{
    if (args.size() != 2) {
    ...
    log::info("HGETALL command executed with key: {}, but WRONGTYPE Operation against a "
              "key holding the wrong kind of value",
              args[1]);
    return std::make_unique<ErrorResponse>(
        "WRONGTYPE Operation against a key holding the wrong kind of value");
