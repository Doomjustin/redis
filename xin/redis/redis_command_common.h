...
struct common_commands {
    ...
    static auto ping(const Arguments& args) -> ResponsePtr;

    // keys pattern
    // return all keys matching pattern
    static auto keys(const Arguments& args) -> ResponsePtr;
    ...
    static auto mget(const Arguments& args) -> ResponsePtr;
    ...
    static auto flushdb(const Arguments& args) -> ResponsePtr;

    // dbsize
    // return the number of keys in the currently-selected database.
    static auto dbsize(const Arguments& args) -> ResponsePtr;

    // expire key seconds
    // Set a timeout on key. After the timeout has expired, the key will automatically be deleted.
    // If the key does not exist, it returns 0. Otherwise, it returns 1.
    static auto expire(const Arguments& args) -> ResponsePtr;
    ...
    static auto ttl(const Arguments& args) -> ResponsePtr;
    ...
    static auto persist(const Arguments& args) -> ResponsePtr;
};
